...
for code, label, digits in countries:
    dial = f"+{digits}"
    if code == "IN":
        entry = (
            f"  PhoneCountryCode(code: '{code}', dialCode: '{dial}', "
            f"label: '{label}', localMinLength: 10, localMaxLength: 10),"
        )
    else:
        entry = (
            f"  PhoneCountryCode(code: '{code}', dialCode: '{dial}', "
...
path = "lib/core/utils/phone_country_codes.dart"
with open(path, "w", encoding="utf-8", newline="\n") as handle:
    handle.write("\n".join(lines))

print(f"Wrote {len(countries)} countries to {path}")
